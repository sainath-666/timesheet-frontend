...
export default function AdminTimesheetReviewPage({
  params,
}: {
  params: { id: string };
}) {
  const id = Number(params.id);
  const ts = adminAllTimesheets.find((t) => t.id === id);
  ...
  return (
    <AppShell role="ADMIN">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-semibold">
            Review Timesheet — {ts.employeeName}
          </h1>
          <p className="text-xs text-slate-400">{ts.periodLabel}</p>
        </div>
        <div className="space-x-2">
          <button className="text-xs px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500">
            Approve (UI only)
          </button>
          <button className="text-xs px-3 py-1.5 rounded-lg bg-rose-600 text-white hover:bg-rose-500">
            Reject (UI only)
          </button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
        <h2 className="text-sm font-semibold mb-3">Entries</h2>
